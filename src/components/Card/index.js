...
const Card = ({data}) => {
  return (
    <div className="relative group overflow-hidden h-full flex flex-col items-center">
      ...
      <Link href={`/anime/${data.mal_id}`} className="absolute z-10 w-full h-full flex items-center justify-center opacity-0 md:group-hover:opacity-100 md:cursor-pointer text-white text-lg font-semibold" rel="noopener noreferrer">See More</Link>
      <Image 
